...
export class DashboardPanel {
    ...
    switch (message.type) {
      ...
      case 'enqueue-command': {
        if ('agent' in message && 'command' in message) {
          ...
          // Enqueueing alone only records the item in memory — it must also
          // be dispatched to Copilot (mirrors the command-palette flow in
          // enqueueCommand.ts), otherwise the item sits at "queued" forever
          // with no log output: exactly the "Send does nothing" symptom.
          // copilotExecutor already surfaces failures via
          // vscode.window.showErrorMessage and marks the queue item failed,
          // so this catch only guards against it rejecting unexpectedly.
          copilotExecutor.executeTask(message.agent, message.command, item.id).catch((error) => {
            logError(`Dashboard enqueue-command dispatch failed for agent "${message.agent}"`, error);
            vscode.window.showErrorMessage(
              `Squad: Failed to run command for ${message.agent}: ${error instanceof Error ? error.message : String(error)}`,
            );
          });
        }
        break;
  ...
  private setupEventListeners(): void {
    ...
    const onCommandCompleted = (data: SquadEvents['command-completed']) => {
      // The event only carries the id and a success/failure flag; look up the
      // full item so the webview can re-render its status, result, and error.
